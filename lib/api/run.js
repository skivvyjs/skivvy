...
function run(task, targetName, packageName, environmentName, projectPath, config) {
	...
	function runNamedTask(taskName, targetName, packageName, environmentName, projectPath, customConfig) {
		return getTask(taskName, packageName, projectPath)
			.then(function(task) {
				var targetConfig = loadTaskConfig(taskName, targetName, packageName, environmentName, projectPath, customConfig);
				var isTaskSet = Array.isArray(targetConfig);
				if (isTaskSet) {
					var targets = targetConfig.map(function(targetConfig) {
						return {
							task: task,
							config: targetConfig
						};
					...
					return runFunctionTask(task, targetConfig);
				}
			});
		...
		function loadTaskConfig(taskName, targetName, packageName, environmentName, projectPath, customConfig) {
			...
			var extendedTargetsConfig = targetsConfig.map(function(targetConfig) {
				...
				var extendedConfig = extendConfig(targetConfig, customConfig, context);
			...
			function extendConfig(baseConfig, customConfig, context) {
				var deepMerge = true;
				var expandedCustomConfig = expandConfigPlaceholders(customConfig, context);
				return extend(deepMerge, {}, baseConfig, expandedCustomConfig);
			}
		}
	}
