...
module.exports = function(options) {
	...
	var api = this;
	return getTaskConfig(taskName, targetName, packageName, environmentName, shouldExpandPlaceholders, api, projectPath);
};

function getTaskConfig(taskName, targetName, packageName, environmentName, shouldExpandPlaceholders, api, projectPath) {
	...
	if (shouldExpandPlaceholders) {
		taskConfig = expandTaskPlaceholders(taskConfig, packageName, environmentName, api, projectPath);
	}
	...
	function expandTaskPlaceholders(taskConfig, packageName, environmentName, api, projectPath) {
		var context = (
				packageName ?
					getExternalTaskContext(packageName, environmentName, api, projectPath)
				:
					getLocalTaskContext(environmentName, api, projectPath)
			);
		return expandConfigPlaceholders(taskConfig, context);


		function getLocalTaskContext(environmentName, api, projectPath) {
		...
		function getExternalTaskContext(packageName, environmentName, api, projectPath) {
			var pkg = loadNpmConfig(projectPath);
			var environmentConfig = api.getEnvironmentConfig({
				environment: environmentName,
				expand: true
