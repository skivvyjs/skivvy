...
var GLOBAL_PACKAGE_PATH_GLOB = MODULE_PREFIX + '*';
var SCOPED_PACKAGE_PATH_GLOB = '@' + '*' + '/' + MODULE_PREFIX + '*';

var SCOPED_PACKAGE_PATH_REGEXP = new RegExp('^(@(?:[^/]+))/' + escapeRegexp(MODULE_PREFIX) + '([^/]+)$');
var GLOBAL_PACKAGE_PATH_REGEXP = new RegExp('^' + escapeRegexp(MODULE_PREFIX) + '([^/]+)$');
...
function getPackages(projectPath, shouldIncludeVersions) {
	...
	function locatePackages(projectPath) {
		return new Promise(function(resolve, reject) {
			...
			var globs = [
				GLOBAL_PACKAGE_PATH_GLOB,
				SCOPED_PACKAGE_PATH_GLOB
			];
	...
	function getPackageName(modulePath, projectPath) {
		...
		return getScopedPackageName(relativePath) || getGlobalPackageName(relativePath);


		function getScopedPackageName(relativePath) {
			var result = SCOPED_PACKAGE_PATH_REGEXP.exec(relativePath);
		...
		function getGlobalPackageName(relativePath) {
			var result = GLOBAL_PACKAGE_PATH_REGEXP.exec(relativePath);
			if (!result) { return null; }
