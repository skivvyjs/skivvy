...
module.exports = function(api) {
	...
	function onInitProjectCompleted(eventData) {
		logSuccess('Initialized project at ' + formatPath(eventData.path));
	}
	...
	function onTaskFailed(eventData) {
		...
		if (task.displayName) {
			...
			logError('Task group failed (' + formatTime(prettyMs(eventData.elapsed)) + ')');
		}
	}
	...
	function onTaskCompleted(eventData) {
		...
		if (task.displayName) {
			...
			logSuccess('Task group completed (' + formatTime(prettyMs(eventData.elapsed)) + ')');
		}
	}
